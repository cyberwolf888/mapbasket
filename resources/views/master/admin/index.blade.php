...
@section('content')
    ...
    <div class="br-pagetitle">
        ...
        <div>
        <h4>Admin</h4>
        <p class="mg-b-0">Manage data admin.</p>
        </div>
    </div><!-- d-flex -->
    ...
    <div class="br-pagebody">
        ...
        <div class="br-section-wrapper">
            <a href="{{ route('master.admin.create') }}" class="btn btn-teal btn-with-icon">
              <div class="ht-40">
                    ...
                    <table id="datatable1" class="table display responsive nowrap">
                            <thead>
                              <tr>
                                ...
                                <th class="wd-45p">Email</th>
                                <th class="wd-45p">Status</th>
                            ...
                            <tbody>
                              ...
                              @foreach ($model as $item)
                                <tr>
                                  ...
                                  <td>{{ $item->name }}</td>
                                  <td>{{ $item->email }}</td>
                                  <td>{{ $item->isActive == 1 ? 'Active' : 'Suspend' }}</td>
                                  <td>
                                      @if($item->id != Auth::user()->id && $item->id != 1)
                                      <a href="{{ route('master.admin.edit', ['id' => $item->id]) }}" class="btn btn-warning btn-icon"><div><i class="fa fa-pencil"></i></div></a>
                                      @endif
                                  </td>
                                  <?php $no++; ?>
                                </tr>
